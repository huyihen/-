...
public class OrderInfoDaoImpl implements IOrderInfoDao{
	...
	public List<OrderInfo> findByPage(String mid, int page, int rows) {
		...
		String sql = "select oid,mid,sid,pid,sname,odate,status,total from orderinfo where mid=? order by odate limit ?, ? ";
		return db.finds(OrderInfo.class, sql,mid, (page-1) * rows, rows);
	}
	@Override
	public List<OrderInfo> findByPagem(String smid, int page, int rows) {
		System.out.println(smid);
		DBHelper db = new DBHelper();
		String sql = "select oid,o.mid as mid,s.mid as smid,o.sid,pid,o.sname,odate,o.status,total from orderinfo o, shopinfo s where o.sid=s.sid and s.mid=? order by odate limit ?, ? ";
		return db.finds(OrderInfo.class, sql,smid, (page-1) * rows, rows);
	}
	@Override
	public int totalm(String mid) {
		String sql = "select count(oid) from orderinfo o, shopinfo s where o.sid=s.sid and s.mid=? ";
		DBHelper db = new DBHelper();
		return db.total(sql,mid);
	}
	@Override
	public int total(String mid) {
		String sql = "select count(oid) from orderinfo where mid=?  ";
		DBHelper db = new DBHelper();
		return db.total(sql,mid);
	}
	...
	public int add(String oid,String mid, String sid, String pid, String total,String sname) {
		...
		return db.update(sql,oid,mid,sid,pid,sname,total);
	}
