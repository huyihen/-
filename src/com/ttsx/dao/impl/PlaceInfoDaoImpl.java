...
public class PlaceInfoDaoImpl implements IPlaceInfoDao{
	...
	@Override
	public int updateStatus(String mid,String pid, String status) {
		DBHelper db = new DBHelper();
		String sql = "update placeinfo set status=1 where mid=?";
		int str=db.update(sql,mid);
		if(str<0) {
			return -1;
		}
		sql = "update placeinfo set status=? where pid=?";
		return db.update(sql,status,pid);
	}
	@Override
	public int update(PlaceInfo pi) {
		DBHelper db = new DBHelper();
		String sql = "update placeinfo set pname=?,tel=?, province=?,city=?, area=?,addr=? where pid=?";
		return db.update(sql,pi.getPname(),pi.getTel(), pi.getProvince(),pi.getCity(),pi.getArea(),pi.getAddr(),pi.getPid());
	}
